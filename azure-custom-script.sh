...
sleep 3
cfy bootstrap /opt/cfy/cloudify-manager-blueprints/simple-manager-blueprint.yaml -i public_ip=$PUBLIC_IP -i private_ip=$PRIVATE_IP -i ssh_user=cfyuser -i ssh_key_filename=/home/$USERNAME/.ssh/key.pem -i ignore_bootstrap_validations=false -i admin_username=admin -i admin_password=admin
...
run_until_success "cfy secrets create mgr_subnet_name -s $4"
run_until_success "cfy secrets create mgr_virtual_network_name -s $5"
run_until_success "cfy secrets create mgr_resource_group_name -s $6"
run_until_success "cfy secrets create location -s $7"
